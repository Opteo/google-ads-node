...
import * as geographicviewserviceModule from '../src';
...
describe('v6.GeographicViewServiceClient', () => {
    it('has servicePath', () => {
        const servicePath = geographicviewserviceModule.v6.GeographicViewServiceClient.servicePath;
        assert(servicePath);
    });

    it('has apiEndpoint', () => {
        const apiEndpoint = geographicviewserviceModule.v6.GeographicViewServiceClient.apiEndpoint;
        assert(apiEndpoint);
    });

    it('has port', () => {
        const port = geographicviewserviceModule.v6.GeographicViewServiceClient.port;
        assert(port);
        assert(typeof port === 'number');
    });

    it('should create a client with no option', () => {
        const client = new geographicviewserviceModule.v6.GeographicViewServiceClient();
        assert(client);
    });
    ...
    it('has initialize method and supports deferred initialization', async () => {
        ...
        assert(client.geographicViewServiceStub);
    });
    ...
    describe('getGeographicView', () => {
        it('invokes getGeographicView without error', async () => {
            ...
            client.initialize();
            const request = generateSampleMessage(new protos.google.ads.googleads.v6.services.GetGeographicViewRequest());
            ...
            const expectedResponse = generateSampleMessage(new protos.google.ads.googleads.v6.resources.GeographicView());
            client.innerApiCalls.getGeographicView = stubSimpleCall(expectedResponse);
            const [response] = await client.getGeographicView(request);
            assert.deepStrictEqual(response, expectedResponse);
            assert((client.innerApiCalls.getGeographicView as SinonStub)
        ...
        it('invokes getGeographicView without error using callback', async () => {
            ...
            const expectedResponse = generateSampleMessage(new protos.google.ads.googleads.v6.resources.GeographicView());
            client.innerApiCalls.getGeographicView = stubSimpleCallWithCallback(expectedResponse);
            const promise = new Promise((resolve, reject) => {
                 client.getGeographicView(
                    request,
                    (err?: Error|null, result?: protos.google.ads.googleads.v6.resources.IGeographicView|null) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(result);
        ...
        it('invokes getGeographicView with error', async () => {
            ...
            client.innerApiCalls.getGeographicView = stubSimpleCall(undefined, expectedError);
            await assert.rejects(client.getGeographicView(request), expectedError);
            assert((client.innerApiCalls.getGeographicView as SinonStub)
                .getCall(0).calledWith(request, expectedOptions, undefined));
        });
    });
