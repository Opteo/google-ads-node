...
function main(customerId, operations) {
  // [START googleads_v23_generated_GoalService_MutateGoals_async]
  /**
  ...
  const {GoalServiceClient} = require('google-ads').v23;

  // Instantiates a client
  const googleadsClient = new GoalServiceClient();

  async function callMutateGoals() {
    // Construct request
    const request = {
      customerId,
      operations,
    ...
    // Run request
    const response = await googleadsClient.mutateGoals(request);
    console.log(response);
  }

  callMutateGoals();
  // [END googleads_v23_generated_GoalService_MutateGoals_async]
}
