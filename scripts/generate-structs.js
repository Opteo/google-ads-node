...
function buildNestedInterfaces(entity) {
  // Skip duplicates
  if (scopedInterfaces.includes(entity.name)) {
    return;
  }
...
function buildInterfaceType(entity) {
  ...
  for (const f in entity.fields) {
    ...
    if (type.includes(`${entity.name},`)) {
      t = t.replace(entity.name, "undefined");
      selfCallingKeys.push(`${entity.name}.${key} = ${type}\n`);
    }
  ...
  for (const selfCallingKey of selfCallingKeys) {
    stream.write(selfCallingKey);
  }
